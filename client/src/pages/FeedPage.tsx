...
export default function FeedPage() {
  ...
  if (error) return <div className="p-8 text-center text-destructive">Error loading feed.</div>;
  ...
  const filteredRumors = rumors?.filter(r => {
    ...
    if (filter === "active") return r.status === "active" || r.status === "inconclusive";
    return true;
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container max-w-4xl py-8 px-4 mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Rumor Feed</h1>
            <p className="text-muted-foreground mt-1">
              Active investigations and verified claims from the campus network.
            </p>
          </div>
          <CreateRumorDialog />
        </div>
        ...
        <Tabs defaultValue="all" onValueChange={setFilter} className="space-y-6">
          <TabsList className="bg-secondary/50 border border-border/50 p-1">
            ...
            <TabsTrigger value="verified" className="data-[state=active]:text-[hsl(var(--status-verified))]">Verified</TabsTrigger>
            <TabsTrigger value="debunked" className="data-[state=active]:text-destructive">Debunked</TabsTrigger>
          </TabsList>

          <div className="grid gap-4">
            {filteredRumors?.length === 0 ? (
              <div className="text-center py-20 border border-dashed border-border rounded-xl">
                <p className="text-muted-foreground">No rumors found in this category.</p>
              </div>
            ) : (
              filteredRumors?.map((rumor) => (
                <Link key={rumor.id} href={`/rumor/${rumor.id}`} className="block group">
                  <Card className="border-border/60 hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 cursor-pointer bg-card/50 backdrop-blur-sm overflow-hidden relative">
                    ...
                    <div className={`absolute left-0 top-0 bottom-0 w-1 ${
                      rumor.trustScore >= 0.8 ? 'bg-[hsl(var(--status-verified))]' : 
                      rumor.trustScore <= 0.2 ? 'bg-destructive' : 'bg-yellow-500'
                    }`} />
                        ...
                        <div className="flex flex-col gap-2 w-full">
                          <div className="flex justify-between items-center w-full">
                            <Badge variant="outline" className="font-mono text-[10px] uppercase tracking-wider bg-background/50">
                              #{rumor.id.toString().padStart(4, '0')}
                            </Badge>
                    ...
                    <CardContent className="pb-4 pl-6">
                      <TrustScore score={rumor.trustScore} />
                    </CardContent>
                    ...
                    <CardFooter className="pt-0 pb-4 pl-6 text-xs text-muted-foreground flex justify-between items-center border-t border-border/30 mt-2 pt-3">
                      <div className="flex gap-4">
                        <span className="flex items-center gap-1.5 hover:text-foreground transition-colors">
                          ...
                          {rumor.viewCount} views
                        </span>
                        <span className="flex items-center gap-1.5 hover:text-foreground transition-colors">
                          <MessageSquare className="h-3.5 w-3.5" />
                          {rumor.evidenceCount} evidence items
                        </span>
                      </div>
