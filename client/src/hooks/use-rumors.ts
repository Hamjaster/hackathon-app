...
export function useRumors() {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(api.rumors.list.path, { credentials: "include" });
...
export function useRumor(id: number) {
  return useQuery({
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your rumor has been anonymously logged to the protocol.",
      });
    },
    onError: (error) => {
      toast({
...
export function useCreateEvidence() {
  ...
  return useMutation({
    mutationFn: async ({ rumorId, ...data }: InsertEvidence & { rumorId: number }) => {
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: [api.rumors.get.path, variables.rumorId] });
      toast({
...
export function useVoteEvidence() {
  ...
  return useMutation({
    mutationFn: async ({ evidenceId, isHelpful, rumorId }: { evidenceId: number, isHelpful: boolean, rumorId: number }) => {
